...
const getUserChannelProfile=asyncHandler(async(req,res)=>{
  ...
  const channel=await User.aggregate([
    ...
    //forth pipeline
    {
      $addFields:{
        ...
        channelsSubscribedToCount:{
          $cond:{
            if:{$in:[req.user?._id,"$subscribers.subscriber"]} ,//$ indicates the field ..check subscriber mai user present h ya nhi
            then:true,
            else:false
          }
        }
    ...
    {
      $project:{
        fullName:1, //jo jo filed select krna h usse 1 put krdo,
        username:1,
        subscibersCount:1,
...
const getWatchHistory=asyncHandler(async(req,res)=>{
  const user=await User.aggregate([
    {
    $match:{
      // _id:req.user._id  it will not work as in mongodb id is stored in string so required to use mongoose as shown below
      _id:new mongoose.Types.ObjectInd(req.user._id)
    }
  },
  {
   $lookup:{
    ...
    pipeline:[
      ...
      {
        //to make array in better way for frontend(optional)
        $addFields:{
          owner:{
            $first:"$owner" //get object "owner" and then . krke extract hone m easy ho jayega
          }
        }
